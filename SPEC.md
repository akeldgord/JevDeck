...
| Original-page viewer | Implemented for the stored original, with the highlight measured from the page's text layer — one rectangle per line of the passage, transformed through the viewport actually in use, so a wrapped passage is not covered by one box that claims the lines between it. A passage that cannot be located is labelled "exact highlight unavailable" rather than drawn approximately. The measurement is exercised without a browser at two zoom levels and on a rotated page (`apps/web/src/lib/excerptGeometry.ts`, `tests/excerptGeometry.test.ts`); no rendered result has been inspected by eye. |
...
| Usage ledger and budget enforcement | Implemented: an append-only ledger, per-user and installation-wide caps, and reserve-then-settle around every provider call so concurrent jobs cannot exceed the available reservation. A charge whose cost could not be established stays counted until an administrator resolves it (`GET /api/admin/budget`, `POST /api/admin/budget/uncertain/:id/reconcile`), the decision is attributed, and a charge above its hold is reported as an incident (`tests/api-r5.test.ts`, `tests/api-budget-admin.test.ts`) |
