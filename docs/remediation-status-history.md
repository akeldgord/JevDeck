...
> **Historical document.** The live status of the `docs/remediation-v3.md` checklist is
> [`remediation-status.md`](./remediation-status.md). Everything in this file is the audit trail it
> grew out of, kept as it was written.

- **Date:** 19–20 September 2026
- **Audited revision:** `0f81317` for sections 1–6 (that revision is on `origin/main`, and the note
  below about the work being uncommitted is historical). §7 is the earlier matrix against
  `docs/remediation-v2.md`, which supplements the original spec and takes precedence where it
  corrects it. Both are the historical record: the live status of the `docs/remediation-v3.md`
  checklist is [`remediation-status.md`](./remediation-status.md).
...
This is the audit trail for `JevDeck_Remediation_Spec.md` v1.0 and its `docs/remediation-v2.md`
addendum: the requirement-to-evidence matrix required by §7 of the specification, the findings with
their evidence, the pass-by-pass record of what was fixed, and the verification performed at the
time. None of it is current status — the live status of the `docs/remediation-v3.md` checklist is
[`remediation-status.md`](./remediation-status.md), and where the two disagree that one is right.
It is kept as it was written rather than retro-edited, because a status document that rewrites its
own history is not evidence. The interim v3 matrix that used to sit at the top of this file was
moved to the live document when it was split out.
...
## 7. Remediation v2 (`docs/remediation-v2.md`) — the v2 matrix as it stood
